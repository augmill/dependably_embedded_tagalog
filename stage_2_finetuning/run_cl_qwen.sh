...
echo "========================================"
echo "Starting Contrastive Learning Training (Qwen-0.5B)"
echo "Fine-tuning Qwen input embeddings with dependency structure"
echo "========================================"
...
OUTPUT_DIR="$REPO_DIR/stage_2_finetuning/cl_qwen_0.5b"

# Training hyperparameters - identical to BERT CL runs
EPOCHS=50
BATCH_SIZE=24
LEARNING_RATE=3e-5
TEMPERATURE=0.07
...
echo ""
echo "Note: Only embed_tokens layer is trainable (all transformer layers frozen)"
echo ""
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    echo "  model = AutoModelForCausalLM.from_pretrained('$OUTPUT_DIR/final_model')"
